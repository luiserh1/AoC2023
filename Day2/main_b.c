...
int main(int argc, const char* argv[])
{
	...
	while (currentGame != NULL)
	{
		...
		while (currentGameSet != NULL)
		{
			if (reqR < currentGameSet->r)
				reqR = currentGameSet->r;

			if (reqG < currentGameSet->g)
				reqG = currentGameSet->g;
		...
		printf("The minimum required for GAME %d are (%d of red, %d of green, %d of blue) "
